...
from src.data.gist import get_gist_index
...
class GistActivations:
    ...
    def from_model_outputs(
        ...
        if cache_all:
            ...
            gist_index = get_gist_index(
                input_ids[0], gist_token, raise_if_no_tokens=True
            )
            num_gist_tokens = gist_index[-1][1]
        ...
        for batch_i, input_row in enumerate(input_ids):
            ...
            if cache_all:
                ...
                selected_indices = list(range(gist_start, gist_end))
                # Keep track of gist starts for the batch.
                gist_indices.append([(gist_start, gist_end)])
            else:
                gist_index = get_gist_index(
                    input_row, gist_token, raise_if_no_tokens=True
                )
                gist_start, gist_end = gist_index[0]
                selected_indices = []
                # For each gist group except the last, take all its gist tokens
                for gist_start, gist_end in gist_index:
                    selected_indices.extend(range(gist_start, gist_end))
                gist_indices.append(gist_index)
