...
from src.data.gist_input_preprocessor import link_custom_collate_chunkaug, gist_attention_preprocessor, meta_link_custom_collate_chunkaug
...
def load_from_disk_then_process(
    ...
    if data_component_name == "qa_link":
        preprocessor_fn = preprocessor.gist_process_qa_link
...
def main():
    ...
    parser.add_argument("--link_num", type=int, default=5, help="Number of link tokens.")
    parser.add_argument(
        "--top_k",
        type=int,
        nargs="+",
    ...
    if len(global_tokenizer) == gist_llama.PRETRAINED_VOCAB_SIZE + 3:
        ...
        link_tokens = []
        for i in range(args.link_num):
            link_tokens.append(f"<link{i+1}>")

        # Initialize gist token
        global_tokenizer.add_special_tokens({"additional_special_tokens": link_tokens}, replace_additional_special_tokens=False)
        ...
        if True:
            with torch.no_grad():
                ...
                avg_embedding = global_model.model.embed_tokens.weight[:-args.link_num].mean(0)
                avg_lm_head = global_model.lm_head.weight[:-args.link_num].mean(0)
                
                # Set both new tokens to the average
                for i in range(args.link_num):
                    global_model.model.embed_tokens.weight[-args.link_num + i] = avg_embedding
                    global_model.lm_head.weight[-args.link_num + i] = avg_lm_head
    else:
    ...
    link_token_ids = global_tokenizer.additional_special_tokens_ids[-args.link_num:]
    link_token = global_tokenizer.additional_special_tokens[-args.link_num:]
    ...
    trainer = Trainer(
        ...
        data_collator = link_custom_collate_chunkaug if meta_gist == 0 else meta_link_custom_collate_chunkaug,
    )
